...
export default function RemixPickerPage() {
  ...
  // ── Fungsi Kamera Di-hoist ke Atas Agar Aman dari Aturan ESLint ──
  const stopWebcam = () => {
  ...
  const handleCaptureAndScan = async () => {
    ...
    if (ctx) {
      ...
      stopWebcam();
      await executeAnalysis(dataUrl.split(",")[1]);
    }
  };

  // Memperbaiki Typo Nama Fungsi yang Menyebabkan Error TypeScript
  ...
  return (
    /* Mengubah max-w-4xl menjadi max-w-6xl agar tampilan dashboard megah & tidak kosong */
    <div className="max-w-6xl mx-auto space-y-6 pb-16 animate-fadeIn">
      ...
      {/* Kontainer Utama Satu Kolom Penuh yang Diperlebar */}
      <div className="w-full min-h-140 bg-white border border-zinc-200 rounded-3xl overflow-hidden relative shadow-xs flex flex-col justify-between">
        <AnimatePresence mode="wait">
          {/* SKENARIO ZERO STATE / INPUT INTERAKTIF */}
          {!result && !loading && (
            <motion.div
              key="input-state"
              ...
              onDrop={handleDrop}
              className={`flex-1 flex flex-col p-6 sm:p-10 relative transition-colors duration-200 ${
                isDragging ? "bg-zinc-50/80" : ""
              }`}
            >
              {/* Menu Tab Internal Atas */}
              ...
              {/* Area Canvas Utama */}
              <div className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto w-full relative min-h-85">
                {activeTab === "camera" ? (
                  isWebcamActive ? (
                    <div className="w-full aspect-square relative rounded-2xl overflow-hidden border border-zinc-200 shadow-sm bg-zinc-950">
                      <video
                        ref={videoRef}
                        autoPlay
                        playsInline
                    ...
                    <div className="text-center space-y-4 py-8">
                      <button
                        type="button"
                        onClick={startWebcam}
                        className="mx-auto h-16 w-16 bg-zinc-50 border border-zinc-200 rounded-2xl flex items-center justify-center text-zinc-400 hover:text-[#1A1A1A] hover:border-zinc-400 transition-all shadow-xs group"
                      >
                        <Camera
                          size={24}
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-zinc-400 font-medium max-w-xs leading-relaxed">
                          Izinkan akses webcam untuk memindai tingkat kesegaran
                          komoditas secara real-time.
                        </p>
                      </div>
                    </div>
                  )
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center">
                    {imagePreview ? (
                      <div className="w-full aspect-square max-w-xs sm:max-w-sm relative rounded-2xl overflow-hidden border border-zinc-200 shadow-xs z-20">
                        <img
                          src={imagePreview}
                          alt="Target Analisis"
                          className="w-full h-full object-cover"
                      ...
                      <div
                        ...
                        className={`w-full border-2 border-dashed rounded-3xl p-8 py-16 flex flex-col items-center justify-center gap-4 cursor-pointer transition-all ${
                          isDragging
                    ...
                    {imagePreview && (
                      <button
                        ...
                        disabled={loading}
                        className="w-full max-w-xs sm:max-w-sm mt-5 bg-[#1A1A1A] text-white font-black py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all text-[10px] uppercase tracking-widest shadow-xs active:scale-99"
                      >
          ...
          {/* LOADING SCREEN DENGAN DETEKSI SCAN */}
          {loading && !result && (
            <div className="flex-1 flex items-center justify-center p-8 relative min-h-[400px]">
              ...
              <PageLoader
                variant="inline"
                message="Mengevaluasi kelayakan pangan AI..."
              />
            </div>
          )}

          {/* PENAMPANG DATA HASIL ANALISIS */}
          {result && !loading && (
            <div className="space-y-6 flex-1 p-6 sm:p-10 animate-fadeIn max-w-4xl mx-auto w-full">
              ...
              {/* Grid Data Analisis untuk memanfaatkan ruang lebar */}
        ...
        {/* Footer Log */}
        <div className="p-4 bg-zinc-50/50 border-t border-zinc-100 flex items-center justify-between text-[9px] font-black tracking-wider text-zinc-400 uppercase">
