...
function scoreColor(score: number) {
  if (score >= 80)
    return { bar: "bg-emerald-400", text: "text-emerald-600", label: "Prima" };
  if (score >= 60)
    return { bar: "bg-amber-400", text: "text-amber-600", label: "Waspada" };
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <p
          className={`text-[10px] font-black uppercase tracking-widest ${accent ? "text-zinc-400" : "text-zinc-400"}`}
        >
          {label}
        </p>
        <p
          className={`text-2xl font-black tracking-tight mt-0.5 ${accent ? "text-white" : "text-[#1A1A1A]"}`}
        >
          {value}
        </p>
        {sub && (
          <p
            className={`text-[11px] font-medium mt-1 ${accent ? "text-zinc-400" : "text-zinc-500"}`}
          >
            {sub}
          </p>
        )}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className="text-xs font-black text-[#1A1A1A] tracking-tight">
              {label}
            </p>
            {badge && (
...
export default function DashboardPage() {
  ...
  const budgetUsed = stats?.profile?.dailyBudgetTarget
    ? Math.min(
        100,
        (stats.totalMoneySaved / (stats.profile.dailyBudgetTarget * 30)) * 100,
      )
    : 0;
      ...
      >
        <div>
          ...
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-[#1A1A1A]">
            Selamat datang, <span className="text-[#EAB308]">{nickname}</span>{" "}
            👋
          </h1>
        ...
        <div className="lg:col-span-5 flex flex-col gap-5">
          ...
          >
            ...
            <div className="flex items-end gap-3 mb-3">
              <span
                className={`text-5xl font-black tracking-tighter ${healthMeta.text}`}
              >
              ...
              <div className="pb-1.5">
                ...
                <p
                  className={`text-[10px] font-black uppercase tracking-wider ${healthMeta.text}`}
                >
                  {healthMeta.label}
                </p>
              </div>
                ...
                {stats.recentJournals.map((j) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`text-[10px] font-black shrink-0 ${sc.text}`}
                      >
                        {j.healthScore}
                      </span>
                    </div>
          ...
          <div className="space-y-2">
            <div className="flex items-end justify-between">
              ...
              <span className="text-[10px] font-bold text-zinc-400 pb-0.5">
                / Rp{" "}
                {(
                  (stats?.profile?.dailyBudgetTarget ?? 30000) * 30
            ...
            {[
              {
                ...
                val: stats?.totalRemix ?? 0,
                label: "Remix",
              },
              {
                icon: <ScanLine size={12} />,
                val: stats?.totalPickerScans ?? 0,
                label: "Scan",
              },
              {
                icon: <BookOpen size={12} />,
                val: stats?.totalJournalEntries ?? 0,
                label: "Jurnal",
              },
            ].map((item) => (
              <div
                key={item.label}
                className="flex flex-col items-center gap-1 py-2 bg-zinc-50 rounded-xl border border-zinc-100"
              >
          ...
          <div className="relative z-10">
            ...
            <div className="mt-4 flex flex-wrap gap-1.5">
              {(stats?.profile?.medicalConditions ?? []).map((c) => (
                <span
                  key={c}
                  className="text-[9px] font-black px-2 py-0.5 bg-sky-500/20 text-sky-300 border border-sky-500/20 rounded-md uppercase tracking-wider"
                >
                  {c}
                </span>
              ))}
              {(stats?.profile?.allergies ?? []).map((a) => (
                <span
                  key={a}
                  className="text-[9px] font-black px-2 py-0.5 bg-amber-500/20 text-amber-300 border border-amber-500/20 rounded-md uppercase tracking-wider"
                >
                  {a}
                </span>
              ))}
              {!stats?.profile?.medicalConditions?.length &&
                !stats?.profile?.allergies?.length && (
                  <span className="text-[10px] text-zinc-500 font-medium">
                    Belum ada kondisi terdaftar.
                  </span>
                )}
            </div>
          </div>
